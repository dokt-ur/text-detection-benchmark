...
class PA_Head(nn.Module):
    ...
    def get_results(self, out, img_meta, cfg):
        ...
        emb = emb.data.cpu().numpy()[0].astype(np.float32)
        ...
        for i in range(1, label_num):
            ...
            score_i = np.mean(score[ind]).item()
            ...
            bbox = bbox.astype('int32').reshape(-1).tolist()
            bboxes.append(bbox)
            scores.append(score_i)
        results = [dict(bboxes=bboxes, scores=scores)]
    ...
    def loss(self, out, gt_texts, gt_kernels, training_masks, gt_instances):
        ...
        # embedding loss
        loss_emb = self.emb_loss(embs, gt_instances, gt_kernels[:, -1, :, :], training_masks, reduce=False)
        losses.update(dict(
            loss_emb=loss_emb
        ))
